...
def create_simple_beat(bpm: int = 120) -> PrettyMIDI:
    ...
    # drums_notes = [Note(velocity=127, pitch=p, start=t, end=t+0.5) for t, p in zip([j*0.5+1 for j in range(4)], pitch_sequence)]
...
# def midi_to_bits(midi_data: PrettyMIDI, pos_per_beat: int = 32) -> np.ndarray:
    # note_positions = midi_data.get_beats()
...
print("Making all notes same duration...")
...
piano_roll = output_midi.get_piano_roll(2)
